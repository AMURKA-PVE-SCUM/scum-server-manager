...
const store = new ElectronStore<AppConfig>({
  defaults: {
    server: {
      serverName: '', serverPath: '', steamCmdPath: '', serverPort: 2302, queryPort: 2502,
...
const lolkaBot = new LolkaBot(store.store.lolkaBot, rconClient, webPanel);
...
function initServices(): void {
  ...
  lolkaBot.updateConfig(store.store.lolkaBot);
  webPanel.setLolkaBot(lolkaBot);
...
app.whenReady().then(() => {
  ...
  // Attach monitoring to an already-running server (e.g. manager restarted
  // while the server kept running), otherwise auto-start a new one if enabled.
  serverManager.monitor().then((found) => {
    if (!found && store.store.server.autoStart) serverManager.start();
  }).catch((e) => console.error('[ServerManager] attach-monitor error:', e.message));

  // Lolka bot auto-start
  if (store.store.lolkaBot.enabled && store.store.lolkaBot.token) {
    lolkaBot.start().catch(e => console.error('[LolkaBot] Auto-start error:', e.message));
  }
});
