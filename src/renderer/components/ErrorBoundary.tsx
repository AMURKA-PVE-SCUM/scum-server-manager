...
export class ErrorBoundary extends Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <Box sx={{ p: 4, bgcolor: '#000000', height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2 }}>
          <Typography variant="h5" sx={{ color: '#f85149' }}>{text.title}</Typography>
          <Box sx={{ bgcolor: '#0a0a0c', p: 2, borderRadius: 1, maxWidth: 600, overflow: 'auto', fontFamily: 'monospace', fontSize: 12, color: '#e6edf3', whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
