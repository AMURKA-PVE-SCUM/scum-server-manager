...
function AppContent() {
  ...
  useEffect(() => {
    ...
    window.electronAPI.config.get().then((cfg: any) => {
      if (alive) setServerName(cfg?.server?.serverName || '');
    }).catch(() => {});
    return () => { alive = false; };
  }, []);
  ...
  const currentItem = navItems.find(i => isActive(i.path));
  const currentTitle = currentItem ? currentItem.label : 'SCUM Server Manager';
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#000000' }}>
      <Box sx={{ px: 2.5, py: 2.5 }}>
        <Typography sx={{
          ...
          background: 'linear-gradient(90deg,#79c0ff,#58a6ff,#f0883e)',
        ...
        {navItems.map((item) => (
          <ListItemButton
            ...
            sx={{
              ...
              '&.Mui-selected': {
                bgcolor: 'linear-gradient(90deg,rgba(88,166,255,0.22),rgba(88,166,255,0.08))',
                backgroundImage: 'linear-gradient(90deg,rgba(88,166,255,0.20),rgba(88,166,255,0.05))',
                color: '#e6edf3',
                boxShadow: 'inset 0 0 0 1px rgba(88,166,255,0.3)',
  ...
  return (
    <Box sx={{ display: 'flex', height: '100vh', bgcolor: '#000000' }}>
      ...
      <Box component="nav" sx={{ width: { md: DRAWER_WIDTH }, flexShrink: { md: 0 } }}>
        <Drawer
          ...
          sx={{
            ...
            '& .MuiDrawer-paper': { width: DRAWER_WIDTH, borderRight: '1px solid #21262d', bgcolor: '#000000' },
          }}
      ...
      <Box component="main" sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', minWidth: 0, height: '100vh', overflow: 'hidden', bgcolor: '#000000' }}>
        <Box sx={{ px: 3, py: 1.5, display: 'flex', alignItems: 'center', gap: 1.5, borderBottom: '1px solid #21262d', background: 'linear-gradient(90deg,rgba(88,166,255,0.07),rgba(0,0,0,0) 40%)' }}>
          <Typography sx={{ fontSize: 17, fontWeight: 700, color: '#e6edf3' }}>
            {currentTitle}
          </Typography>
          <Box sx={{ flex: 1 }} />
          <Typography sx={{ fontSize: 12.5, color: serverName ? '#58a6ff' : '#8b949e', fontWeight: serverName ? 600 : 400 }}>
            {serverName || 'SCUM Server Manager'}
          </Typography>
        </Box>
        <Box sx={{ flex: 1, overflow: 'auto', p: 3, bgcolor: '#000000' }}>
          <Routes>
