...
export function GameSettings() {
  ...
  const load = async () => {
    ...
    try {
      const d = await window.electronAPI.files.read(`${cfg.server.serverPath}/SCUM/Saved/Config/WindowsServer/GameUserSettings.ini`);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await window.electronAPI.files.write(`${config.server.serverPath}/SCUM/Saved/Config/WindowsServer/GameUserSettings.ini`, stringify(iniData));
      setSnack({ open: true, message: t('gameSettings', 'saved'), severity: 'success' });
  ...
  return (
    <Box>
      <Typography variant="h4" sx={{ mb: 3, fontWeight: 'bold' }}>{t('gameSettings', 'title')}</Typography>
      ...
      <Button variant="contained" startIcon={<SaveIcon />} onClick={handleSave} disabled={saving}>{t('common', 'save')}</Button>
