...
export class SteamCmd {
  ...
  /**
   * Запуск steamcmd через .bat-файл. Консоль не прячем — окно steamcmd видно.
   * cmd.exe — консольное приложение, поэтому ему выделяется новое окно консоли
   * (piped/inherit из Electron, у которого нет консоли, окно не создаёт).
   */
  private spawnSteamCmd(args: string[], captureOutput: boolean, onLine?: (line: string) => void): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const cmdLine = `${q(exe)} ${args.map((a) => q(a).replace(/%/g, '%%')).join(' ')}`;
      fs.writeFileSync(
        batchPath,
        `@chcp 65001 >nul\r\n@echo off\r\n${cmdLine}\r\nexit /b %ERRORLEVEL%\r\n`,
        'utf8',
      ...
      const batchProc = spawn('cmd.exe', ['/c', 'start', '', '/wait', batchPath], {
      ...
      const t = setTimeout(() => {
        ...
        reject(Object.assign(new Error('SteamCMD timeout (15 min)'), { exitCode: -1 }));
      }, TIMEOUT);

      batchProc.on('error', (e) => { clearTimeout(t); reject(e); });
      batchProc.on('close', (code) => {
        clearTimeout(t);
        console.log(`[SteamCMD] process closed with code=${code}`);
        if (code === 0 || code === 7) resolve('');
        else reject(Object.assign(new Error(`SteamCMD exit code ${code}`), { exitCode: code }));
      });
    });
  }
