...
export class WebPanel {
  ...
  private pluginsConfig: PluginsConfig = {
    ...
    chatSender: 'AMUR bot',
    autoMessages: { enabled: false, onlyWhenOnline: true, messages: [] },
    onlineChat: { enabled: false, intervalSec: 300, template: '👥 Онлайн: {online}/{max}' },
    joinLeaveChat: { enabled: false, joinTemplate: '✅ {player} зашёл на сервер', leaveTemplate: '❌ {player} вышел' },
    ratingBlacklist: [],
  };
  ...
  private playerSessions = new Map<string, { connectedAt: number; rewardedHours: number }>();
  private lastTopRewardTime = 0;
  private autoMessagesTimer: NodeJS.Timeout | null = null;
  private autoMessageLastSent: number[] = [];
  private onlineChatTimer: NodeJS.Timeout | null = null;
  ...
  start(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.server = http.createServer((req, res) => {
        ...
        } else if (url === '/api/plugins/auto-messages' && method === 'GET') {
          this.sendJson(res, this.pluginsConfig.autoMessages);
        } else if (url === '/api/plugins/auto-messages' && method === 'POST') {
          this.handleSetAutoMessages(req, res);
        } else if (url === '/api/plugins/online-chat' && method === 'GET') {
          this.sendJson(res, this.pluginsConfig.onlineChat);
        } else if (url === '/api/plugins/online-chat' && method === 'POST') {
          this.handleSetOnlineChat(req, res);
        } else if (url === '/api/plugins/joinleave-chat' && method === 'GET') {
          this.sendJson(res, this.pluginsConfig.joinLeaveChat);
        } else if (url === '/api/plugins/joinleave-chat' && method === 'POST') {
          this.handleSetJoinLeaveChat(req, res);
        ...
        } else if (url === '/api/scumdb/status' && method === 'GET') {
          this.handleScumDbStatus(res);
        } else if (url === '/api/scumdb/players' && method === 'GET') {
          this.handleScumDbPlayers(res);
        } else if (url === '/api/scumdb/squads' && method === 'GET') {
          this.handleScumDbSquads(res);
        } else if (url.match(/^\/api\/scumdb\/squad-members\/(\d+)$/) && method === 'GET') {
          const squadId = parseInt(url.match(/^\/api\/scumdb\/squad-members\/(\d+)$/)![1], 10);
          this.handleScumDbSquadMembers(squadId, res);
        } else if (url.match(/^\/api\/scumdb\/player\/(\d+)$/) && method === 'GET') {
          const steamId = url.match(/^\/api\/scumdb\/player\/(\d+)$/)![1];
          this.handleScumDbPlayer(steamId, res);
        } else if (url.match(/^\/api\/scumdb\/steam-profile\/(\d+)$/) && method === 'GET') {
          const steamId = url.match(/^\/api\/scumdb\/steam-profile\/(\d+)$/)![1];
          this.handleScumDbSteamProfile(steamId, res);
        ...
        } else if (url === '/api/lolkabot/status' && method === 'GET') {
          this.handleLolkaBotStatus(res);
        } else if (url === '/api/lolkabot/start' && method === 'POST') {
          this.handleLolkaBotStart(res);
        } else if (url === '/api/lolkabot/stop' && method === 'POST') {
          this.handleLolkaBotStop(res);
        } else if (url === '/api/lolkabot/config' && method === 'GET') {
          this.handleLolkaBotConfig(res);
        } else if (url === '/api/lolkabot/config' && method === 'POST') {
          this.handleLolkaBotSaveConfig(req, res);
        } else {
          res.writeHead(404);
          res.end('Not found');
        }
      ...
      this.server.listen(this.config.port, '0.0.0.0', () => {
        ...
        this.startAutoMessagesTimer();
        this.startOnlineChatTimer();
        resolve();
      });
    });
  }
  ...
  stop(): void {
    ...
    this.stopAutoMessagesTimer();
    this.stopOnlineChatTimer();
  ...
  private async handleGiveCurrency(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    try {
      ...
      let cmd: string;
      if (type === 'gold') cmd = `ChangeCurrencyBalance Gold +${amount} ${steamId}`;
      else if (type === 'fame') cmd = `ChangeFamePoints +${amount} ${steamId}`;
      else cmd = `ChangeCurrencyBalance Normal +${amount} ${steamId}`;
  ...
  private async handleSetAutoMessages(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    try {
      const body = await this.readBody(req);
      const cfg = JSON.parse(body);
      this.pluginsConfig.autoMessages = cfg;
      if (this.pluginsSaveCallback) this.pluginsSaveCallback(this.pluginsConfig);
      this.restartAutoMessagesTimer();
      this.sendJson(res, { success: true });
    } catch (e: any) {
      this.sendJson(res, { error: e.message }, 500);
    }
  ...
  private async handleSetOnlineChat(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    try {
      const body = await this.readBody(req);
      const cfg = JSON.parse(body);
      this.pluginsConfig.onlineChat = cfg;
      if (this.pluginsSaveCallback) this.pluginsSaveCallback(this.pluginsConfig);
      this.restartOnlineChatTimer();
      this.sendJson(res, { success: true });
    } catch (e: any) {
      this.sendJson(res, { error: e.message }, 500);
    }
  ...
  private async handleSetJoinLeaveChat(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    try {
      const body = await this.readBody(req);
      const cfg = JSON.parse(body);
      this.pluginsConfig.joinLeaveChat = cfg;
      if (this.pluginsSaveCallback) this.pluginsSaveCallback(this.pluginsConfig);
      this.sendJson(res, { success: true });
    } catch (e: any) {
      this.sendJson(res, { error: e.message }, 500);
  ...
  private handleLolkaBotStatus(res: http.ServerResponse): void {
    if (!this.lolkaBot) { this.sendJson(res, { running: false }); return; }
    this.sendJson(res, { running: this.lolkaBot.isRunning() });
  }

  private async handleLolkaBotStart(res: http.ServerResponse): Promise<void> {
    if (!this.lolkaBot) { this.sendJson(res, { error: 'LolkaBot not initialized' }, 500); return; }
    try {
      await this.lolkaBot.start();
      this.sendJson(res, { success: true });
    } catch (e: any) {
      this.sendJson(res, { error: e.message }, 500);
    }
  ...
  private handleLolkaBotStop(res: http.ServerResponse): void {
    if (!this.lolkaBot) { this.sendJson(res, { error: 'LolkaBot not initialized' }, 500); return; }
    this.lolkaBot.stop();
    this.sendJson(res, { success: true });
  }

  private handleLolkaBotConfig(res: http.ServerResponse): void {
    if (!this.lolkaBot) { this.sendJson(res, { enabled: false, token: '', guildId: '', channelId: '', activityText: 'AMURKA PVE' }); return; }
    const status = this.lolkaBot.getStatus();
    this.sendJson(res, status.config);
  }

  private async handleLolkaBotSaveConfig(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    if (!this.lolkaBot) { this.sendJson(res, { error: 'LolkaBot not initialized' }, 500); return; }
    try {
      const body = await this.readBody(req);
      const cfg: LolkaBotConfig = JSON.parse(body);
      this.lolkaBot.updateConfig(cfg);
      const full = this.serverConfigProvider?.get() || {};
      full.lolkaBot = cfg;
      if (this.serverConfigProvider?.save(full)) {
        this.sendJson(res, { success: true });
      } else {
        this.sendJson(res, { error: 'Save failed' }, 500);
      }
    } catch (e: any) {
      this.sendJson(res, { error: e.message }, 500);
    }
  ...
  private async handleSetRewards(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    try {
      ...
      if (this.pluginsSaveCallback) this.pluginsSaveCallback(this.pluginsConfig);
      this.startRewardTimer(); // restart timer respecting new enabled state
      this.sendJson(res, { success: true });
    } catch (e: any) {
      this.sendJson(res, { error: e.message }, 500);
    }
  ...
  private async handleRewardsData(res: http.ServerResponse): Promise<void> {
    ...
    for (const [sid, session] of this.playerSessions) {
      const hoursPlayed = Math.floor((now - session.connectedAt) / 3600000);
      data[sid] = { lastRewardTime: session.rewardedHours > 0 ? now : 0, hoursPlayed };
    }
  ...
  private startRewardTimer(): void {
    ...
    if (!this.pluginsConfig.rewards.enabled) {
  ...
  private async checkRewards(): Promise<void> {
    ...
    // Hourly rewards — session-based: отсчёт идёт с момента подключения игрока
    if (cfg.hourlyEnabled) {
      for (const [sid, session] of this.playerSessions) {
        const elapsedHours = Math.floor((now - session.connectedAt) / 3600000);
        const hoursToReward = elapsedHours - session.rewardedHours;
        if (hoursToReward < 1) continue;

        const player = this.cachedPlayers.find(p => p.steamId === sid);
        const name = player?.name || sid;

        const cmds: string[] = [];
        const delta = cfg.hourlyMoney * hoursToReward;
        if (delta > 0) cmds.push(`ChangeCurrencyBalance Normal +${Math.round(delta)} ${sid}`);
        const gdelta = cfg.hourlyGold * hoursToReward;
        if (gdelta > 0) cmds.push(`ChangeCurrencyBalance Gold +${Math.round(gdelta)} ${sid}`);
        const fdelta = cfg.hourlyFame * hoursToReward;
        if (fdelta > 0) cmds.push(`ChangeFamePoints +${Math.round(fdelta)} ${sid}`);
        for (const cmd of cmds) {
          await this.rconClient!.sendCommand(cmd);
        }
        const parts: string[] = [];
        if (cfg.hourlyMoney > 0) parts.push(`$${cfg.hourlyMoney * hoursToReward}`);
        if (cfg.hourlyGold > 0) parts.push(`${cfg.hourlyGold * hoursToReward} золота`);
        if (cfg.hourlyFame > 0) parts.push(`${cfg.hourlyFame * hoursToReward} славы`);
        await this.rconClient.sendCommand(`SendChat 2 "⏱ ${name} провёл на сервере ${hoursToReward}ч и получил: ${parts.join(', ')}"`);
        console.log(`[Rewards] Hourly: ${name} +${hoursToReward}h (money:${cfg.hourlyMoney}, gold:${cfg.hourlyGold}, fame:${cfg.hourlyFame})`);
        session.rewardedHours = elapsedHours;
      }
    }
      ...
      if (this.lastTopRewardTime === 0 || elapsedDays >= cfg.topIntervalDays) {
        ...
        if (topPlayers.length > 0) {
          for (let i = 0; i < topPlayers.length; i++) {
            ...
            // Issue delta commands directly (atomic, no race condition)
            const cmds: string[] = [];
            if (cfg.topMoney > 0) cmds.push(`ChangeCurrencyBalance Normal +${Math.round(cfg.topMoney * multiplier)} ${sid}`);
            if (cfg.topGold > 0) cmds.push(`ChangeCurrencyBalance Gold +${Math.round(cfg.topGold * multiplier)} ${sid}`);
            if (cfg.topFame > 0) cmds.push(`ChangeFamePoints +${Math.round(cfg.topFame * multiplier)} ${sid}`);
            for (const cmd of cmds) {
              await this.rconClient!.sendCommand(cmd);
            }
            const tparts: string[] = [];
            if (cfg.topMoney > 0) tparts.push(`$${cfg.topMoney * multiplier}`);
            if (cfg.topGold > 0) tparts.push(`${cfg.topGold * multiplier} золота`);
            if (cfg.topFame > 0) tparts.push(`${cfg.topFame * multiplier} славы`);
            await this.rconClient.sendCommand(`SendChat 2 "🏆 ${p.playerName} занял ${i + 1} место и получил: ${tparts.join(', ')}"`);
          }
  ...
  // Auto Messages timer
  private startAutoMessagesTimer(): void {
    this.stopAutoMessagesTimer();
    this.autoMessageLastSent = [];
    this.autoMessagesTimer = setInterval(() => this.checkAutoMessages(), 10000);
  }

  private stopAutoMessagesTimer(): void {
    if (this.autoMessagesTimer) { clearInterval(this.autoMessagesTimer); this.autoMessagesTimer = null; }
  }

  private restartAutoMessagesTimer(): void {
    if (this.autoMessagesTimer) this.startAutoMessagesTimer();
  }

  private async checkAutoMessages(): Promise<void> {
    if (!this.rconClient || !this.rconClient.isConnected()) return;
    const cfg = this.pluginsConfig.autoMessages;
    if (!cfg || !cfg.enabled || !cfg.messages.length) return;
    if (cfg.onlyWhenOnline && this.cachedPlayers.length === 0) return;
    const now = Date.now();
    if (!this.autoMessageLastSent.length) {
      this.autoMessageLastSent = cfg.messages.map(() => 0);
    }
    for (let i = 0; i < cfg.messages.length; i++) {
      const msg = cfg.messages[i];
      if (!msg.enabled || !msg.text) continue;
      const lastSent = this.autoMessageLastSent[i] || 0;
      if (now - lastSent < msg.intervalSec * 1000) continue;
      this.autoMessageLastSent[i] = now;
      const text = msg.text.replace(/"/g, "'").trim();
      if (text) {
        await this.rconClient.sendCommand(`SendChat 2 "${text}"`);
      }
    }
  }

  // Online Chat timer
  private startOnlineChatTimer(): void {
    this.stopOnlineChatTimer();
    const cfg = this.pluginsConfig.onlineChat;
    if (!cfg || !cfg.enabled) return;
    this.onlineChatTimer = setInterval(() => this.checkOnlineChat(), cfg.intervalSec * 1000);
  }

  private stopOnlineChatTimer(): void {
    if (this.onlineChatTimer) { clearInterval(this.onlineChatTimer); this.onlineChatTimer = null; }
  }

  private restartOnlineChatTimer(): void {
    this.stopOnlineChatTimer();
    this.startOnlineChatTimer();
  }

  private async checkOnlineChat(): Promise<void> {
    if (!this.rconClient || !this.rconClient.isConnected()) return;
    const cfg = this.pluginsConfig.onlineChat;
    if (!cfg || !cfg.enabled) return;
    try {
      ...
      const players = this.parseListPlayersOutput(r.response);
      const count = players.length;
      const text = cfg.template.replace('{online}', String(count)).replace(/{max}/, '?').replace(/"/g, "'").trim();
      if (text) {
        await this.rconClient.sendCommand(`SendChat 2 "${text}"`);
      }
    } catch {}
  ...
  async sendJoinLeaveChat(playerName: string, type: 'join' | 'leave'): Promise<void> {
    if (!this.rconClient || !this.rconClient.isConnected()) return;
    const cfg = this.pluginsConfig.joinLeaveChat;
    if (!cfg || !cfg.enabled) return;
    const template = type === 'join' ? cfg.joinTemplate : cfg.leaveTemplate;
    const text = template.replace(/{player}/g, playerName).replace(/"/g, "'").trim();
    if (text) {
      try {
        await this.rconClient.sendCommand(`SendChat 2 "${text}"`);
      } catch {}
  ...
  private parsePlayerEvents(line: string): void {
    ...
    if (possessedMatch) {
      ...
      this.sendJoinLeaveChat(playerName, 'join');
      return;
    }
    ...
    if (loginMatch) {
      ...
      this.sendJoinLeaveChat(name, 'join');
      return;
    }
  ...
  private async pollPlayers(): Promise<void> {
    ...
    try {
      ...
      if (result.success && result.response) {
        ...
        // Track player sessions for hourly rewards
        for (const sid of onlineIds) {
          if (!this.playerSessions.has(sid)) {
            this.playerSessions.set(sid, { connectedAt: Date.now(), rewardedHours: 0 });
          }
        }
        // Clean up sessions for disconnected players
        for (const [sid] of this.playerSessions) {
          if (!onlineIds.has(sid)) this.playerSessions.delete(sid);
        }
  ...
  private getDbReader(): ScumDatabaseReader | null {
    if (!this.serverPath) return null;
    const reader = new ScumDatabaseReader(this.serverPath);
    if (!reader.isAvailable()) { reader.close(); return null; }
    return reader;
  }

  private async handleScumDbStatus(res: http.ServerResponse): Promise<void> {
    const reader = this.getDbReader();
    if (!reader) { this.sendJson(res, { ok: false, error: 'SCUM.db not found' }); return; }
    try {
      const path = reader['dbPath'] || '';
      const ok = reader.isAvailable();
      this.sendJson(res, { ok, path: ok ? path : null, error: ok ? undefined : 'SCUM.db not found' });
    } finally { reader.close(); }
  }

  private async handleScumDbPlayers(res: http.ServerResponse): Promise<void> {
    const reader = this.getDbReader();
    if (!reader) { this.sendJson(res, { ok: false, error: 'SCUM.db not found', players: [] }); return; }
    try {
      await initSqlJs();
      reader.open();
      const players = reader.listPlayers();
      this.sendJson(res, { ok: true, players });
    } catch (e: any) {
      this.sendJson(res, { ok: false, error: e.message, players: [] });
    } finally { reader.close(); }
  }

  private async handleScumDbSquads(res: http.ServerResponse): Promise<void> {
    const reader = this.getDbReader();
    if (!reader) { this.sendJson(res, { ok: false, error: 'SCUM.db not found', squads: [] }); return; }
    try {
      await initSqlJs();
      reader.open();
      const squads = reader.listSquads();
      this.sendJson(res, { ok: true, squads });
    } catch (e: any) {
      this.sendJson(res, { ok: false, error: e.message, squads: [] });
    } finally { reader.close(); }
  }

  private async handleScumDbSquadMembers(squadId: number, res: http.ServerResponse): Promise<void> {
    const reader = this.getDbReader();
    if (!reader) { this.sendJson(res, { ok: false, error: 'SCUM.db not found', members: [] }); return; }
    try {
      await initSqlJs();
      reader.open();
      const members = reader.listSquadMembers(squadId);
      this.sendJson(res, { ok: true, members });
    } catch (e: any) {
      this.sendJson(res, { ok: false, error: e.message, members: [] });
    } finally { reader.close(); }
  }

  private async handleScumDbPlayer(steamId: string, res: http.ServerResponse): Promise<void> {
    const reader = this.getDbReader();
    if (!reader) { this.sendJson(res, { ok: false, error: 'SCUM.db not found' }); return; }
    try {
      await initSqlJs();
      reader.open();
      const detail = reader.getPlayerDetailWithAttrs(steamId);
      this.sendJson(res, { ok: !!detail, player: detail, error: detail ? undefined : 'Player not found' });
    } catch (e: any) {
      this.sendJson(res, { ok: false, error: e.message });
    } finally { reader.close(); }
  }

  private async handleScumDbSteamProfile(steamId: string, res: http.ServerResponse): Promise<void> {
    try {
      const url = `https://steamcommunity.com/profiles/${steamId}/?xml=1`;
      const xml = await this.fetchUrl(url);
      const name = (xml.match(/<steamID><!\[CDATA\[(.*?)\]\]><\/steamID>/) || xml.match(/<steamID>(.*?)<\/steamID>/))?.[1] || '';
      const cd = (re: RegExp) => { const m = xml.match(re); return m?.[1] || ''; };
      const avatarIcon = cd(/<avatarIcon><!\[CDATA\[(.*?)\]\]><\/avatarIcon>/) || cd(/<avatarIcon>(.*?)<\/avatarIcon>/);
      const avatarMedium = cd(/<avatarMedium><!\[CDATA\[(.*?)\]\]><\/avatarMedium>/) || cd(/<avatarMedium>(.*?)<\/avatarMedium>/);
      const avatarFull = cd(/<avatarFull><!\[CDATA\[(.*?)\]\]><\/avatarFull>/) || cd(/<avatarFull>(.*?)<\/avatarFull>/);
      this.sendJson(res, { ok: true, steamId, name, avatarIcon, avatarMedium, avatarFull });
    } catch (e: any) {
      this.sendJson(res, { ok: false, error: e.message });
    }
  }

  private fetchUrl(url: string, timeoutMs = 10000, maxBytes = 1024 * 1024): Promise<string> {
    return new Promise((resolve, reject) => {
      const prot = url.startsWith('https') ? require('https') : require('http');
      const req = prot.get(url, (response: any) => {
        let data = '';
