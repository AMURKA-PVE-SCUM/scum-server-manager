  ...
  private async executeItem(item: WargmCardItem, steamId: string): Promise<{ success: boolean; message: string }> {
    ...
    switch (item.type) {
      ...
      case 'money': {
        ...
        const cmd = `ChangeCurrencyBalance Normal +${Math.round(amount)} ${steamId}`;
        const r = await this.rconClient!.sendCommand(cmd);
        return { success: r.success, message: r.success ? `Деньги +${amount}` : `Деньги: ${r.response || 'ОШИБКА'}` };
      }

      case 'gold': {
        const amount = parseInt(d.amount) || 0;
        if (amount <= 0) return { success: false, message: 'Золото: неверное количество' };
        const cmd = `ChangeCurrencyBalance Gold +${Math.round(amount)} ${steamId}`;
        const r = await this.rconClient!.sendCommand(cmd);
        return { success: r.success, message: r.success ? `Золото +${amount}` : `Золото: ${r.response || 'ОШИБКА'}` };
      }

      case 'fame': {
        const amount = parseInt(d.amount) || 0;
        if (amount <= 0) return { success: false, message: 'Слава: неверное количество' };
        const cmd = `ChangeFamePoints +${Math.round(amount)} ${steamId}`;
        const r = await this.rconClient!.sendCommand(cmd);
        return { success: r.success, message: r.success ? `Слава +${amount}` : `Слава: ${r.response || 'ОШИБКА'}` };
      }
